...
pub(crate) fn export_gexf_ocdg(g: &Ocdg, log: &Ocel, file_path: &str) -> Result<bool, Box<dyn Error>> {
    ...
    for (src, edge_data) in &g.irels {
        for (tar, rels) in edge_data {
            ...
            for (r, events) in rels {
                let ev_s: Vec<String> = events.iter().map(|eid| log.events.get(eid).unwrap().eid.clone()).collect();
...
pub(crate) fn export_gexf_ocdg_pretty(g: &Gexf, file_path: &str) -> Result<bool, Box<dyn Error>> {

    let serde_ocel = serde_json::to_string_pretty(&g).unwrap();
    let output_file = OpenOptions::new().create(true).write(true).truncate(true).open(file_path).unwrap();
